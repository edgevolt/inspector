/**
 * SentinelOne Log Parser
 * Parses SentinelOne EDR logs (Deep Visibility, Threats) in JSON format
...
function summarizeObjectArray(arr, fieldName) {
    ...
    // Specific formatting for SentinelOne object arrays
    if (lowerName.includes('indicators')) {
        return arr.map(item => item.IndicatorName || item.name || JSON.stringify(item)).join('; ');
    }
...
/**
 * Detect the SentinelOne event sub-type from a parsed JSON object.
 */
function detectLogSubType(obj) {
    ...
    if (obj.ThreatName || obj.Classification || obj.MitigationStatus || obj.AnalystVerdict) {
        return 'Threat / Incident';
    }

    if (obj.SrcIp || obj.DstIp || obj.DstPort || obj.TransportProtocol) {
    ...
    if (obj.RegistryKeyPath || obj.RegistryValueName) {
        return 'Registry Activity';
    }

    if (obj.FilePath || obj.OldFilePath) {
        return 'File Activity';
    }

    if (obj.ProcessName || obj.ProcessCmd || obj.ProcessImagePath) {
...
/**
 * Parse a single SentinelOne log entry.
 */
export function parseLog(logLine) {
    ...
    flattened._log_source = 'SentinelOne';
...
export function getSummaryStats(parsedLogs) {
    ...
    return {
        ...
        uniqueAgents: getUniqueValues(parsedLogs, 'AgentId').length,
        uniqueSites: getUniqueValues(parsedLogs, 'SiteName').length,
