/**
 * Sophos Log Parser
 * Parses Sophos Central and Intercept X endpoint logs (JSON Format)
...
/**
 * Detect the Sophos event sub-type from a parsed JSON object.
 */
function detectLogSubType(obj) {
    if (!obj || typeof obj !== 'object') return 'Unknown';

    if (obj.type && typeof obj.type === 'string' && obj.type.startsWith('Event::Endpoint::')) {
        return obj.type.split('::').pop() || obj.type;
    }
    ...
    // Inference based on Sophos distinct properties
    if (obj.threat || obj.severity || obj.mitigation_action) {
        return 'Alert / Threat';
    }

    if (obj.remote_ip || obj.local_port || obj.direction || obj.url) {
        return 'Network Activity';
    }

    if (obj.process_name || obj.cmd_line || obj.parent_name) {
        return 'Process Activity';
    }

    if (obj.path) {
        return 'File / Registry Activity';
    }
...
/**
 * Parse a single Sophos log entry.
 */
...
export function getSummaryStats(parsedLogs) {
    ...
    return {
        ...
        uniqueEndpoints: getUniqueValues(parsedLogs, 'endpoint_id').length,
        logSubTypes: countFieldValues(parsedLogs, '_logSubType'),
        threats: countFieldValues(parsedLogs, 'threat')
    };
}
