/**
 * Palo Alto Networks Cortex XDR Log Parser
 * Parses Cortex XDR endpoint and network logs (JSON / XQL Event Format)
 * Supports flattening nested event structures and categorizing event sub-types (e.g. PROCESS, NETWORK, FILE, REGISTRY, ALERTS)
 */
...
/**
 * Detect the Cortex XDR event sub-type from a parsed JSON object.
 */
function detectLogSubType(obj) {
    if (!obj || typeof obj !== 'object') return 'Unknown';

    if (obj.event_type) {
        return obj.event_type.toUpperCase();
    }

    // Inference based on Palo Alto Cortex XDR distinct properties
    if (obj.alert_name || obj.incident_id || obj.alert_id) {
        return 'INCIDENT / ALERT';
    }

    if (obj.action_remote_ip || obj.action_local_port || obj.action_network_protocol) {
        return 'NETWORK';
    }

    if (obj.action_registry_key_name || obj.action_registry_value_name) {
        return 'REGISTRY';
    }

    if (obj.action_file_path || obj.action_file_name) {
        return 'FILE';
    }

    if (obj.action_process_command_line || obj.action_process_image_path || obj.actor_process_image_path || obj.actor_process_command_line) {
        return 'PROCESS';
    }
...
/**
 * Parse a single Cortex XDR log entry.
 */
export function parseLog(logLine) {
    ...
    flattened._log_source = 'Palo Alto Cortex XDR';
...
export function getSummaryStats(parsedLogs) {
    ...
    return {
        ...
        uniqueAgents: getUniqueValues(parsedLogs, 'agent_id').length,
        logSubTypes: countFieldValues(parsedLogs, '_logSubType'),
        alerts: countFieldValues(parsedLogs, 'alert_name')
    };
}
